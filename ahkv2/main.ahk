#Include xpoe.ahk
#Include lib\wincapture\dxgi.ahk
...
; 参数配置窗口
g1 := RangeSelectWindow("清空背包", Func1Event, "Ctrl+Alt+Num1:菜单`nNum1:开始`nNum0:中断", 5, 11)
if c != "" and c.HasProp("func1") {
    g1.result := c.func1
}
g2 := RangeSelectWindow("10个一组", Func2Event, "Ctrl+Alt+Num2:菜单`nNum2:开始`nNum0:中断", 5, 2)
if c != "" and c.HasProp("func2") {
    g2.result := c.func2
}
g3 := RangeSelectWindow("庄园通货转换", Func3Event, "Ctrl+Alt+Num3:菜单`nNum3:开始`nNum0:中断", 5, 2)
if c != "" and c.HasProp("func3") {
    g3.result := c.func3
}
items := ["知识卷轴", "蜕变石", "改造石", "增幅石", "重铸石", "目标装备"]
g4 := ItemPositionWindow("自动改造", items, Func4Event, "Ctrl+Alt+Num4:菜单`nNum4:开始`nNum0:中断")
if c != "" and c.HasProp("func4") {
    g4.results := c.func4
}

items2 := ["制图钉", "知识卷轴", "重铸石", "点金石", "瓦尔宝珠"]
g5_1 := RangeSelectWindow("自动瓦图", Func5Event1, "Ctrl+Alt+Num5:菜单`nNum5:开始`nNum0:中断", 5, 11)
g5_2 := ItemPositionWindow("自动瓦图", items2, Func5Event2, "Ctrl+Alt+Num5:菜单`nNum5:开始`nNum0:中断")
    ...
    Loop cols {
        ...
        Loop rows {
            ...
            if first {
                ...
                MoveMouseRandomBezier(center.x, center.y, , 10, 5)
            }
            LeftClickRandom(50, 25, 25, 10)
        }
        reversed := !reversed
    }
    KeyUpSleep()
